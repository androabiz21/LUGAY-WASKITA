  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500 pb-20 px-0 md:px-10 pt-8 bg-stone-950 min-h-screen text-stone-100">
      <header className="space-y-4 px-4">
        <button onClick={() => onNavigate(AppView.HOME)} className="flex items-center gap-2 text-stone-500 hover:text-amber-500 transition-colors mb-6 group"><div className="p-2 rounded-full group-hover:bg-amber-900/20 transition-colors"><HomeIcon size={18} /></div><span className="font-bold uppercase tracking-widest text-[10px]">Beranda</span></button>
        <div className="flex items-center gap-4">
          <div className="p-4 bg-stone-900 border border-stone-800 rounded-2xl text-white shadow-2xl shadow-black"><LayersIcon size={32} /></div>
          <div>
          ...
          <div className="p-8 bg-stone-900/40 rounded-[40px] border border-stone-800 space-y-8 shadow-inner backdrop-blur-md">
            <div className="grid grid-cols-1 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-stone-500 uppercase tracking-widest flex items-center gap-2 px-1"><UserIcon size={12} className="text-amber-600" /> Nama Lengkap Sesuai Sanad</label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-stone-500 uppercase tracking-widest flex items-center gap-2 px-1"><CalendarIcon size={12} className="text-amber-600" /> Tgl Lahir (Opsional)</label>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-stone-500 uppercase tracking-widest flex items-center gap-2 px-1"><HeartIcon size={12} className="text-rose-600" /> Nama Ibu (Opsional)</label>
            ...
            <div className={`aspect-[3/4] rounded-[40px] border-2 border-dashed flex flex-col items-center justify-center transition-all overflow-hidden relative bg-stone-950 ${image || isCameraActive ? 'border-amber-600/30' : 'border-stone-800'}`}>
              {!image && !isCameraActive && (
                <div className="text-center p-8 space-y-6">
                  <div className="w-20 h-20 bg-stone-900 rounded-3xl flex items-center justify-center text-stone-700 mx-auto border border-stone-800"><CameraIcon size={40} /></div>
              ...
              {isCameraActive && (
                <div className="absolute inset-0">
                  ...
                  <div className="absolute bottom-8 left-0 right-0 flex justify-center items-center gap-6 z-20">
                    <button onClick={toggleCamera} className="p-4 bg-black/40 backdrop-blur-md rounded-full text-stone-300 border border-stone-700"><SwitchIcon size={24} /></button>
                    <button onClick={capturePhoto} className="w-16 h-16 bg-amber-600 rounded-full flex items-center justify-center text-stone-950 shadow-lg active:scale-90"><FlameIcon size={28} /></button>
                    <button onClick={stopCamera} className="p-4 bg-black/40 backdrop-blur-md rounded-full text-stone-300 border border-stone-700 hover:bg-rose-900"><RefreshIcon size={24} /></button>
                  </div>
                </div>
              )}
              {image && (
                <div className="absolute inset-0 group">
                  ...
                  <button onClick={() => { setImage(null); startCamera(); }} className="absolute top-6 right-6 p-4 bg-stone-900/80 backdrop-blur rounded-full text-stone-100 shadow-xl hover:bg-amber-600 transition-all opacity-0 group-hover:opacity-100"><RefreshIcon size={20} /></button>
                </div>
              )}
          ...
          <div className="p-0 md:p-12 glass-panel md:rounded-[60px] border-y md:border border-stone-800 bg-stone-900/30 min-h-[500px] flex flex-col shadow-2xl">
            <div className="flex items-center justify-between border-b border-stone-800 pb-6 pt-8 px-6 md:pt-0 md:px-0 relative z-10">
              <div className="flex items-center gap-3 text-amber-500">
                <FlameIcon size={24} className="animate-pulse" />
              ...
              {isVisualizing && (
                <div className="flex items-center gap-2 px-3 py-1 bg-amber-600/20 rounded-full border border-amber-600/30 animate-pulse">
                  <LoaderIcon size={10} className="animate-spin text-amber-500" />
            ...
            <div className="flex-1 overflow-y-auto relative z-10">
              ...
              {loading && !analysis && (
                <div className="h-full flex flex-col items-center justify-center space-y-8 py-20 px-6 text-center">
                  <div className="relative"><div className="w-24 h-24 border-2 border-amber-900 rounded-full animate-ping"></div><FlameIcon className="absolute inset-0 m-auto text-amber-600 animate-pulse" size={40} /></div>
                  ...
                  {khodamVisual ? (
                    <div className="px-4 md:px-0 animate-in zoom-in duration-1000">
                      <div className="p-1 bg-stone-900 border border-stone-800 md:rounded-[32px] overflow-hidden shadow-[0_0_40px_-10px_rgba(217,119,6,0.3)] flex flex-col">
                         ...
                         <div className="p-6 text-center border-t border-stone-800 bg-stone-950 flex justify-between items-center">
                            ...
                            <button onClick={handleDownload} className="flex items-center gap-2 px-5 py-2.5 bg-amber-600 hover:bg-amber-700 text-stone-950 rounded-xl text-[9px] font-black uppercase tracking-widest shadow-lg transition-all active:scale-95"><DownloadIcon size={14} /> SIMPAN</button>
                         </div>
                      </div>
                    </div>
                  ) : isVisualizing ? (
                    <div className="px-4 md:px-0">
                      <div className="aspect-[3/4] md:aspect-[4/5] bg-stone-950/50 md:rounded-[32px] border border-stone-800 border-dashed flex flex-col items-center justify-center space-y-4">
                         <LoaderIcon className="animate-spin text-amber-600" size={32} />
                         <p className="text-amber-600/60 font-heritage italic text-sm">Menata dimensi visual batin...</p>
                      </div>
                    </div>
                  ) : !loading && analysis && (
                    <div className="px-4 md:px-0 animate-in slide-in-from-top duration-700">
